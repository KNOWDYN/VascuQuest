...
- scopes all durable evidence by the exact code revision, preventing reuse of a bundle generated by a different commit;
- writes the final durable report under:

```text
MyDrive/VascuQuest/parameterized_cohort_qualification/<code-revision-prefix>/parameterized-cohort-qualification.json
```
