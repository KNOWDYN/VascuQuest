...
This is a release gate, not an ordinary CI test. It acquires only the exact
canonical artifacts used by the shipped core backend, verifies their manifest
checksums, validates source alignment/structure, exercises representative
public-API reads and the shipped flow-rate reconstruction, and emits a
machine-readable validation report.

Dense path-resolved artifacts, alternate MATLAB/WFDB representations,
``model_variations``, and plausibility metadata are outside the claimed core v1
scope and are recorded explicitly as exclusions.
"""
...
def _package_versions() -> dict[str, str]:
    return {
        ...
        for name in ("vascuquest", "numpy", "platformdirs", "typer")
    }
...
def _artifact_inventory(
    ...
    for artifact_id in CORE_ARTIFACT_IDS:
        ...
        _require(
            inspection.state.value == "verified",
            f"{artifact_id} is not checksum-verified",
        )
        _require(
            inspection.observed_checksum == artifact.checksum_value,
            f"{artifact_id} checksum differs from canonical manifest",
        )
...
def _validate_scalar_sources(
    ...
    for quantity in schema.quantities:
        for mapping in quantity.source_mappings:
            ...
            checked_mappings += 1
    _require(checked_mappings > 0, "no implemented scalar schema mappings were checked")
    ...
    for subject_id in EXPECTED_SUBJECT_IDS:
        ...
        _require(
            not cell.missing and cell.value is not None,
            f"age is missing for subject {subject_id}",
        )
        age = int(cell.value)
        _require(float(age) == cell.value, f"non-integral age for subject {subject_id}")
        _require(age in age_groups, f"unexpected age {age} for subject {subject_id}")
        age_groups[age].append(subject_id)
    for age, subject_ids in age_groups.items():
        _require(
            len(subject_ids) == 729,
            f"age {age} must contain 729 subjects, found {len(subject_ids)}",
        )
    ...
    return age_groups, {
        ...
        "age_group_counts": {
            str(age): len(subject_ids) for age, subject_ids in age_groups.items()
        },
    }
...
def _stratified_subjects(age_groups: dict[int, list[str]]) -> tuple[str, ...]:
    ...
    _require(
        len(selected) == 18 and len(set(selected)) == 18,
        "invalid deterministic age/configuration geometry sample",
    )
...
def _validate_geometry(
    ...
    observed = {
        name
        for name in members
        if name.startswith("pwdb_geo_") and name.endswith(".csv")
    }
    _require(
        observed == expected,
        "geometry archive subject inventory is not exactly 1..4374",
    )
    ...
    for subject_id in sampled_subjects:
        ...
        _require(
            result.quantity.canonical_name == "vascular_geometry",
            "wrong geometry quantity identity",
        )
        _require(result.evidence is EvidenceClass.SOURCE, "geometry must be SOURCE")
        _require(
            result.subject is not None
            and result.subject.canonical_subject_id == subject_id,
            "geometry subject mismatch",
        )
        ...
        _require(
            ...
            f"duplicate geometry segment for subject {subject_id}",
        )
        _require(
            all(
                segment.length_m > 0
                and segment.inlet_radius_m > 0
                and segment.outlet_radius_m > 0
                for segment in segments
            ),
            f"non-positive source geometry length/radius for subject {subject_id}",
        )
...
def _validate_waveform_archive(waveform_path: Path) -> dict[str, object]:
    expected_names = {
    ...
    with zipfile.ZipFile(waveform_path, "r") as archive:
        ...
        for info in archive.infolist():
            ...
            if basename in expected_names:
                _require(
                    basename not in by_name,
                    f"ambiguous waveform ZIP member {basename!r}",
                )
                by_name[basename] = info
        _require(
            set(by_name) == expected_names,
            "common-site archive does not expose all 13 x 4 declared members",
        )

        for basename in sorted(expected_names):
            with archive.open(by_name[basename], "r") as raw:
                ...
                _require(
                    header and header[0] == "Subject Number",
                    f"{basename} lacks Subject Number",
                )
                sample_fields = header[1:]
                _require(sample_fields, f"{basename} has no waveform samples")
                _require(
                    sample_fields
                    == tuple(f"pt{index}" for index in range(1, len(sample_fields) + 1)),
                ...
                for values in reader:
                    ...
                    _require(
                        ...
                        f"{basename} contains a malformed row",
                    )
                    _require(
                        row_index < len(EXPECTED_SUBJECT_IDS),
                        f"{basename} contains more than 4374 subject rows",
                    )
                    _require(
                        values[0].strip() == EXPECTED_SUBJECT_IDS[row_index],
                        f"{basename} subject alignment fails at row {row_index + 2}",
                    )
                    row_index += 1
                _require(
                    row_index == 4374,
                    f"{basename} contains {row_index} subjects instead of 4374",
                )
...
def _validate_full_subject_result(result: object, quantity_name: str) -> None:
    _require(
        result.quantity.canonical_name == quantity_name,
        f"quantity identity mismatch for {quantity_name}",
    )
    _require(
        result.evidence is EvidenceClass.SOURCE,
        f"{quantity_name} must remain SOURCE evidence",
    )
    _require(
        result.dimensions == ("subject",),
        f"{quantity_name} must return a subject axis",
    )
    _require(
        len(result.coordinates) == 1 and result.coordinates[0].name == "subject",
        f"{quantity_name} subject coordinate missing",
    )
    _require(
        tuple(result.coordinates[0].values) == EXPECTED_SUBJECT_IDS,
        f"{quantity_name} subject coordinate is misaligned",
    )
    _require(
        len(tuple(result.values)) == 4374,
        f"{quantity_name} does not contain 4374 values",
    )
    _require(bool(result.provenance_ref), f"{quantity_name} lacks provenance")
...
def _validate_public_api(
    ...
    _require(
        session.capabilities() == EXPECTED_CAPABILITIES,
        "public core capability set differs from release scope",
    )
    status = session.status()
    _require(
        status.path_resolved_supported is False,
        "path-resolved support must remain unavailable in core-only release",
    )
    _require(
        tuple(subject.canonical_subject_id for subject in session.subjects())
    ...
    _require(
        location_ids == tuple(PWDB_MEASUREMENT_SITE_IDS),
        "public measurement-site inventory mismatch",
    )

    scalar_results: dict[str, object] = {}
    for quantity_name in LOCATION_FREE_SCALARS:
    ...
    for quantity_name, site_id in FIXED_SITE_SCALARS.items():
        result = session.get(quantity_name, location=MeasurementSite(site_id))
        _validate_full_subject_result(result, quantity_name)
        summary = _result_summary(result)
        summary["measurement_site"] = site_id
        scalar_results[quantity_name] = summary
    ...
    for age, site_id, signal in zip(EXPECTED_AGES, sites, signals, strict=True):
        ...
        _require(
            waveform.quantity.canonical_name == signal,
            f"waveform quantity mismatch for {signal}",
        )
        _require(
            waveform.evidence is EvidenceClass.SOURCE,
            f"{signal} waveform must remain SOURCE",
        )
        _require(
            waveform.subject is not None
            and waveform.subject.canonical_subject_id == subject_id,
            "waveform subject mismatch",
        )
        _require(
            getattr(waveform.location, "canonical_site_id", None) == site_id,
            "waveform site mismatch",
        )
        ...
        _require(
            ...
            "waveform values must be a non-empty vector",
        )
        _require(times.shape == values.shape, "waveform time/value shape mismatch")
        _require(
            waveform.time_coordinate.unit == "s",
            "waveform time coordinate must use seconds",
        )
        if times.size > 1:
            _require(
                np.allclose(
                ...
                "waveform time spacing is not 500 Hz",
            )
        missing = np.asarray(waveform.missing_mask, dtype=bool)
        padding = np.asarray(waveform.padding_mask, dtype=bool)
        _require(
            missing.shape == values.shape and padding.shape == values.shape,
            "waveform masks do not match values",
        )
        _require(
            not np.any(missing & padding),
            "waveform missing/padding masks overlap",
        )
    ...
    velocity = session.waveform(
        "flow_velocity",
        subject=reconstruction_subject,
        location=reconstruction_site,
    )
    area = session.waveform(
        "luminal_area",
        subject=reconstruction_subject,
        location=reconstruction_site,
    )
    ...
    expected = np.asarray(velocity.values, dtype=float) * np.asarray(
        area.values,
        dtype=float,
    )
    ...
    _require(
        ...
        "real-source flow-rate reconstruction differs from Q=U*A",
    )
    _require(
        flow_rate.evidence is EvidenceClass.RECONSTRUCTED,
        "flow-rate evidence must be RECONSTRUCTED",
    )
    _require(flow_rate.canonical_unit == "m^3/s", "flow-rate unit mismatch")
    _require(
        flow_rate.method_id == FLOW_RATE_RECONSTRUCTION_ID,
        "flow-rate method identity mismatch",
    )
    ...
    return {
        ...
        "schema_quantities": [
            quantity.canonical_name for quantity in schema.quantities
        ],
    }
...
def _validate_licence_and_attribution(repo_root: Path) -> dict[str, object]:
    ...
    _require('license = "Apache-2.0"' in pyproject, "package licence metadata mismatch")
    _require(
        "Apache License" in license_text and "Version 2.0" in license_text,
        "LICENSE is not Apache-2.0 text",
    )
    _require(
        "10.5281/zenodo.3275625" in readme,
        "README lacks canonical dataset attribution",
    )
    _require(
        "10.1152/ajpheart.00218.2019" in readme,
        "README lacks authoritative PWDB article attribution",
    )
    _require(
        "does not relicense the source dataset" in readme,
        "README does not preserve the external dataset licence boundary",
    )
    return {
...
def run_validation(
    ...
    try:
        artifact_paths, artifacts = _artifact_inventory(acquirer, manifest)
        age_groups, scalar = _validate_scalar_sources(artifact_paths, schema)
        ...
        report["geometry_validation"] = _validate_geometry(
            artifact_paths["geometry"],
            session,
            _stratified_subjects(age_groups),
        )
        ...
        report["licence_and_attribution"] = _validate_licence_and_attribution(
            repo_root
        )
...
def main() -> int:
    ...
    print(
        json.dumps(
            {"status": report["status"], "report": str(args.report)},
            sort_keys=True,
        )
    )
    return 0
